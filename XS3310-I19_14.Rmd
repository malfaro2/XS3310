...
> Lema de Neyman-Pearson: Supongamos que se desea contrastar las hipótesis simples $H_{0}: \theta = \theta_0$ contra $H_{1}: \theta = \theta_1$ basados en una muestra aleatoria de una población con distribución que depende de $\theta$. Además, sea $\mathcal{L}(\theta)$ la función de verosimilitud de la muestra para un valor de $\theta$. Para un tamaño dado $\alpha_0$, el contraste que minimiza $\beta(\delta)$ tiene una región crítica determinada por:
		$RC_{\delta^*} = \left\lbrace \mathbf{X} | \frac{\mathcal{L}(\theta_0)}{\mathcal{L}(\theta_1)} < k \right\rbrace $
...
_______________________________________________________
> Contraste uniformemente más potente: Un contraste de tamaño $\alpha_0$ es \textbf{uniformemente más potente} (UMP) para contrastar $H_{0}:\theta = \theta_0$ contra $H_{1}: \theta \in \Omega_{1}$ si es más potente para $H_{0}:\theta = \theta_0$ contra $H_{1}: \theta = \theta_1$, para todo $\theta_1 \in \Omega_{1}$.
...
_______________________________________________________
> Cociente de verosimilitudes monótono: Una muestra aleatoria con función de verosimilitud $\mathcal{L}(\theta)$ tiene un \textbf{cociente de verosimilitudes monótono} (CVM) en un estadístico $T = T(X_{1}, X_{2}, ... , X_{n})$ si, para $\theta_0 < \theta_1$, el cociente $\frac{\mathcal{L}(\theta_0)}{\mathcal{L}(\theta_1)}$ es una función monótona de $T$. 
...
Contrastes de hipótesis - bootstrap, debate de valores p.
