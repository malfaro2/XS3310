...
En resumen: ¿qué es un estimador puntual y qué propiedades hacen que sea aceptable para estimar un parámetro? ¿cuáles son los métodos para encontrar un estimador puntual con todas las características que queremos?

# ¿Qué vamos a discutir hoy?

Dos métodos para encontrar estimadores: 

El método de momentos que hace uso de momentos muestrales y poblaciones. 

El método de máxima verosimilitud que es uno de los más utilizados en práctica.

---

# Estimadores de momentos
	
> Definición 1.13. Momento al origen. Si $X$ es una variable aleatoria con distribución conocida, se define el *k*-ésimo momento al origen como: $\mu_{k}^{\prime} = E\left[X^{k}\right]$
	
> Definición 1.14. Momento central. Si $X$ es una variable aleatoria con distribución conocida y media $\mu$, se define el *k*-ésimo momento central como:
$\mu_{k} = E\left[(X-\mu)^{k}\right]$

Para este tema también vamos a definir un nuevo tipo de momento, que llamaremos momento muestral:
	
> Definición 1.15. Momento muestral. Si $X_{1}, X_{2}, ... , X_{n}$ es una muestra aleatoria de una distribución conocida, entonces se define el \textit{k}-ésimo momento muestral como:
...
![Figura 4. Verosimilitud de una $Unif(0,\theta)$ a partir de una muestra aleatoria.](./maxvero.jpg)	
...
9.82 Sea $Y_1, Y_2, . . . , Y_n$ una muestra aleatoria de la función de densidad dada por 

$f (y | \theta) = \frac{1}{\theta} ry^{r−1}\exp{-y^r/\theta}$, $\theta > 0, y > 0$, o 0 en cualquier otro punto, donde r es una constante positiva conocida.

a. Encuentre un estadístico suficiente para $\theta$.
b. Encuentre el MLE de $\theta$.
c. ¿El estimador del inciso b es un MVUE para $\theta$?
