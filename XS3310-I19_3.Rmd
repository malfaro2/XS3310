...
> Definición 1.5. Sea $X_{1}, X_{2}, ... , X_{n}$ una muestra aleatoria tal que la distribución de probabilidad de $X_j$, $\forall j$, depende de un parámetro $\theta$; sea $\hat{\theta}$ un estimador de $\theta$. Se dice que $\hat{\theta}$ es un \underline{estimador insesgado} de $\theta$ si cumple

$$E(\hat{ \theta })=\theta$$

En el caso contrario se dice que es un estimador sesgado.

Ejemplo: Sea $X_{1}, X_{2}, ... , X_{n}$ una muestra aleatoria sobre una población con media $\mu$ y variancia $\sigma^2$, pruebe que $\overline{X}$ y $S^2$ son estimadores insesgados para estimar $\mu$ y $\sigma^2$ respectivamente.

Solución: Se ha demostrado previamente que $E\left(\overline{X}\right)=\mu$ por lo que podemos concluir que $\overline{X}$ es insesgado para estimar $\mu$, esto para cualquier tipo de población. Anteriormente se había demostrado para una población Normal que $S^2$ también es insesgado para $\sigma^2$ pero, ahora debemos demostrarlo para el caso de cualquier distribución:
---

# Estimador Insesgado
	
$E\left(S^2\right)=E\left[\frac{\sum{\left(X_{j}-\overline{X}\right)^2}}{n-1}\right]=\frac{1}{n-1}E\left[\sum{\left[\left(X_{j}-\mu\right)-\left(\overline{X}-\mu\right)\right]^2}\right]$
	
$=\frac{1}{n-1}E\left[\sum{\left[\left(X_{j}-\mu\right)^{2}-2\left(X_{j}-\mu\right)\left(\overline{X}-\mu\right) + \left(\overline{X}-\mu\right)^{2} \right]}\right]$
	
$=\frac{1}{n-1}E\left[\sum{\left(X_{j}-\mu\right)^{2}}-2\left(\overline{X}-\mu\right)\sum{\left(X_{j}-\mu\right)} + \sum{\left(\overline{X}-\mu\right)^{2}}\right]$
	
$=\frac{1}{n-1}E\left[\sum{\left(X_{j}-\mu\right)^{2}}-2n\left(\overline{X}-\mu\right)^2 + n\left(\overline{X}-\mu\right)^{2}\right]$
	
$=\frac{1}{n-1}\left[\sum{E\left[\left(X_{j}-\mu\right)^{2}\right]}-nE\left[\left(\overline{X}-\mu\right)^2\right]\right]$
	
$=\frac{1}{n-1}\left(n\sigma^{2} - n\frac{\sigma^2}{n}\right)=\frac{1}{n-1}\left(n\sigma^{2} - \sigma^{2}\right)=\frac{\sigma^{2}\left(n-1\right)}{n-1}=\sigma^2$
...
De este resultado podemos obtener de una vez la función de distribución de una Potencial:
...
ADEMÁS: Se puede construir un estimador insesgado de $\theta$ a partir de $X_{\left(n\right)}$, de modo que 
...
$E(\hat{\theta}^{*}) = E\left[\frac{\left(n+1\right)X_{\left(n\right)}}{n}\right] = \frac{n+1}{n}E(X_{\left(n\right)}) = \frac{n+1}{n} \cdot \frac{n\theta}{n+1} = \theta$
	
---

# Estimador Insesgado

Ejemplo: Sea $X_{1}, X_{2}, ... , X_{n}$ una muestra aleatoria tal que $X_{j} \sim Pareto\left(2,\beta\right)$. Determine si $X_{\left(1\right)}$ es un estimador insesgado para $\beta$. 
...
> Definición 1.7. Estimadores asintóticamente insesgados. Algunos estimadores, aunque no son insesgados, cumplen que $\lim_{n \to +\infty}{B(\hat{\theta})} =0$, es decir, el sesgo tiende a cero cuando n tiende a infinito. A estos estimadores se les llama asintóticamente insesgados. 
...
a. $\lim_{n \to +\infty}{B(X_{\left(n\right)})} = \lim_{n \to +\infty}{\frac{-\theta}{n+1}} = 0$

b. $\lim_{n \to +\infty}{B(X_{\left(1\right)})} = \lim_{n \to +\infty}{\frac{\beta}{2n-1}} = 0$

Se cumple que ambos estimadores son asintóticamente insesgados para sus respectivos parámetros.
	
NOTA: También se cumple que un estimador $\hat{\theta}$ es asintóticamente insesgado para $\theta$ si $\lim_{n\rightarrow\infty}{E(\hat{\theta})} = \theta$.
...
> Definicion 1.8. Error cuadrático medio. Supongamos que $\hat{\theta}$ es un estimador de un parámetro $\theta$, se define el \underline{error cuadrático medio} de $\hat{\theta}$, denotado como $ECM(\hat{\theta})$, por $ECM(\hat{\theta}) = E\left[\left(\hat{\theta}-\theta\right)^2\right]$

> Teorema 1.22. Si $\hat{\theta}$ es un estimador de $\theta$, entonces
$ECM(\hat{\theta}) = Var(\hat{\theta}) + \left[B(\hat{\theta})\right]^2$

Prueba:

$ECM(\hat{\theta}) = E\left[\left(\hat{\theta}-\theta\right)^2\right] = E\left[\left(\left[\hat{\theta} - E(\hat{\theta})\right] + \left[E(\hat{\theta}) - \theta \right]\right)^2\right] = E\left[\left(\left[\hat{\theta} - E(\hat{\theta})\right] + B(\hat{\theta})\right)^2\right]$
...
Ejemplo: Si $X_{1}, X_{2}, ... , X_{n}$ es una muestra aleatoria tal que $X_{j} \sim Pareto \left(2,\beta\right)$ y $X_{\left(1\right)}$ es un estimador de $\beta$, determine el error cuadrático medio de $X_{\left(1\right)}$. 
	
Solución: Se demostró que $X_{(1)} \sim Pareto\left(2n,\beta\right)$ y que $B\left(X_{\left(1\right)}\right) = \frac{\beta}{2n-1}$. También sabemos que $Var\left(X_{\left(1\right)}\right) = \frac{2n\beta^{2}}{\left(2n-1\right)^{2}\left(2n-2\right)}$. Por lo tanto,
	
$ECM(X_{\left(1\right)}) = \frac{2n\beta^{2}}{\left(2n-1\right)^{2}\left(2n-2\right)} + \frac{\beta^2}{\left(2n-1\right)^2} = \frac{4n\beta^{2} - 2\beta^{2}}{\left(2n-1\right)^{2}\left(2n-2\right)} = \frac{\beta^2}{\left(2n-1\right)\left(n-1\right)}$

---

# Eficiencia
	
La eficiencia de un estimador está relacionada a la variabilidad de dicho estimador, la cual se ve representada por el error cuadrático medio. Empezaremos con un ejemplo que compare la variabilidad entre dos estimadores: 
...
Próxima clase: consistente y suficiente. Asignación del proyecto 1.
